...
def register():
    ...
    user = User(email=email, full_name=full_name, password_hash=hash_password(password), role=Role(role), organization=org)
    db.session.add(user)
    db.session.commit()
...
def login():
    ...
    token = create_access_token(identity={"user_id": user.id, "role": user.role.value})
    return {"access_token": token, "user": {"id": user.id, "email": user.email, "role": user.role.value, "full_name": user.full_name}}
...
def me():
    ...
    return {"id": u.id, "email": u.email, "role": u.role.value, "full_name": u.full_name}
