...
def verify_token():
    """Verifica token e retorna/cria dados do usuário - VERSÃO CORRIGIDA PARA FORNECEDORES"""
    if not db:
        return jsonify({'erro': 'Serviço indisponível'}), 503
    ...
    try:
        ...
        tipo_usuario = data.get('tipo', 'requisitante')  # NOVO: Recebe tipo do frontend
        ...
        if users and len(users) > 0:
            # Usuário existe
            user_data = users[0].to_dict()
            user_doc_id = users[0].id
            logger.info(f"✅ Usuário encontrado: {email}, perfil: {user_data.get('perfil')}")
        else:
            # CRIAR NOVO USUÁRIO COM TIPO CORRETO
            logger.info(f"⚠️ Criando novo usuário: {email} como {tipo_usuario}")
            
            # Determinar perfil baseado no email ou tipo
            perfil = 'fornecedor' if tipo_usuario == 'fornecedor' else 'requisitante'
            
            # Se o email contém padrões de fornecedor
            if '@empresa' in email or '@fornecedor' in email or 'cnpj' in email.lower():
                perfil = 'fornecedor'
            
            user_data = {
                'email': email,
                'nome': decoded.get('name', email.split('@')[0]),
                'perfil': perfil,  # USA O PERFIL CORRETO
                'ativo': True,
            ...
            # Se for fornecedor, criar também na coleção Fornecedores
            if perfil == 'fornecedor':
                db.collection('Fornecedores').document(uid).set({
                    'email': email,
                    'razaoSocial': decoded.get('name', email.split('@')[0]),
                    'cnpj': data.get('cnpj', ''),  # Recebe CNPJ do frontend se enviado
                    'processos': [],
                    'propostas': [],
                    'ativo': True,
                    'criadoEm': firestore.SERVER_TIMESTAMP
...
def cadastrar_fornecedor():
    """Cadastra novo fornecedor no Firebase"""
    try:
        data = request.get_json()
        
        # Validar dados obrigatórios
        required = ['email', 'senha', 'razaoSocial', 'cnpj']
        ...
        new_user = auth.create_user(
            email=data['email'],
            password=data.get('senha', '123456'),
            display_name=data['razaoSocial']
        )
        
        # Criar no Firestore com perfil FORNECEDOR
        db.collection('Usuario').document(new_user.uid).set({
            'email': data['email'],
            ...
            'perfil': 'fornecedor',  # ✅ IMPORTANTE: Define como fornecedor
            'ativo': True,
            'criadoEm': firestore.SERVER_TIMESTAMP,
            'criadoPor': request.user['uid']
        })
        
        # Criar também na coleção Fornecedores
        db.collection('Fornecedores').document(new_user.uid).set({
...
@app.route('/api/fornecedor/registrar', methods=['POST'])
def registrar_fornecedor():
    """Registra novo fornecedor via Firebase (público)"""
    try:
        data = request.get_json()
        ...
        # Salvar no Firestore - Coleção Usuario
        db.collection('Usuario').document(new_user.uid).set({
            'email': data['email'],
            'nome': data['razaoSocial'],
            'perfil': 'fornecedor',  # SEMPRE FORNECEDOR
            'ativo': True,
            'criadoEm': firestore.SERVER_TIMESTAMP
        })
        
        # Salvar na coleção Fornecedores
        db.collection('Fornecedores').document(new_user.uid).set({
            'razaoSocial': data['razaoSocial'],
            'cnpj': data['cnpj'],
            'email': data['email'],
        ...
        logger.error(f"Erro ao registrar fornecedor: {e}")
