...
#
# Essas rotas permitem que um usuário autenticado com perfil de
# administrador ou comprador crie novas contas no sistema. Novos
# usuários são gravados na tabela `usuarios` com a senha criptografada
# (bcrypt). Nenhuma rota existente foi modificada, garantindo que as
# demais funcionalidades permaneçam intactas.
...
def criar_usuario():
    ...
    Requer autenticação e perfil do solicitante como 'admin' ou
    'comprador'. O corpo da requisição deve conter 'nome',
    'email', 'senha' e 'perfil' (requisitante, comprador ou
    fornecedor). Retorna 201 e os dados do usuário criado em caso de
    sucesso.
    """
    try:
        # Permitir apenas administradores ou compradores
        if request.perfil not in ('comprador', 'admin'):
            return jsonify({'message': 'Apenas administradores ou compradores podem criar usuários'}), 403
        ...
        # Validação básica
        if not nome or not email or not senha or perfil not in ('requisitante', 'comprador', 'fornecedor'):
            return jsonify({'message': 'Campos nome, email, senha e perfil são obrigatórios'}), 400
