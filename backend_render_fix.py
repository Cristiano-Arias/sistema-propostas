...
def get_user_profile():
    ...
    try:
        ...
        # Buscar usuário por email na coleção usuarios
...
def criar_fornecedor():
    ...
    try:
        ...
        users = db.collection('usuarios').where('email', '==', requester_email).get()
        if not users or len(users) == 0:
            return jsonify({'erro': 'Usuário não encontrado'}), 404
        ...
        db.collection('usuarios').document(new_user.uid).set(user_doc)
