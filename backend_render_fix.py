...
class MockDB:
    def __init__(self):
        self.users = {
            'wesley.lopes@forteoliveira.com': {
                'email': 'wesley.lopes@forteoliveira.com',
                'nome': 'Wesley Lopes',
                'perfil': 'fornecedor',
                'ativo': True
            },
            'admggrengenharia@gmail.com': {
    ...
    def collection(self, name):
        return MockCollection(self.users if name == 'Usuario' else {})
...
class MockCollection:
    ...
    def stream(self):
        return [MockDoc(email, data) for email, data in self.data.items()]
...
class MockQuery:
    ...
    def get(self):
        if self.field == 'email' and self.op == '==':
            if self.value in self.data:
                return [MockDoc(self.value, self.data[self.value])]
        return []
...
def initialize_firebase():
    """Inicializa Firebase com credenciais - versão com fallback para mock"""
    global db
    ...
    if os.path.exists('credentials.json'):
        ...
        try:
            ...
            # Verificar se é um arquivo válido (não placeholder)
            if cred_dict.get('private_key_id') == 'temp_key_id':
                print("⚠️ Credenciais são placeholder, usando modo mock")
                cred_dict = None
                
        except Exception as e:
            print(f"❌ Erro ao ler credentials.json: {e}")
    
    # Se encontrou credenciais válidas, tentar inicializar Firebase
    if cred_dict:
        try:
            ...
            print(error_msg)
            print("🔄 Fallback para modo mock...")
    
    # Fallback para modo mock
    print("🎭 Inicializando em modo MOCK para desenvolvimento")
    db = MockDB()
    logger.info("✅ Modo mock inicializado com usuários de teste")
    print("✅ Modo mock inicializado com usuários de teste:")
    print("   - wesley.lopes@forteoliveira.com (fornecedor)")
    print("   - admin@sistema.com (admin)")
    print("   - comprador@empresa.com (comprador)")
...
def listar_fornecedores():
    ...
    try:
        if not db:
            # Modo simulação - retorna dados fake
            fornecedores_fake = [
                {
                    'id': 'fornecedor_1',
                    'email': 'wesley.lopes@forteoliveira.com',
                    'nome': 'Wesley Lopes',
                    'empresa': 'Forte Oliveira',
                    'status': 'ativo',
                ...
                },
                {
                    ...
                    'status': 'ativo',
                    'criadoEm': '2025-01-02T00:00:00Z'
                }
            ]
            return jsonify(fornecedores_fake), 200
        
        # Modo real com Firebase
        fornecedores = []
        for doc in db.collection('fornecedores').stream():
            fornecedor_data = doc.to_dict()
            fornecedor_data['id'] = doc.id
...
def criar_fornecedor():
    """
    Simula a criação de um fornecedor para compatibilidade com o frontend.
    
    Como o Firebase Admin SDK não está configurado, esta rota retorna sucesso
    para permitir que o frontend continue funcionando. Em produção, seria
    necessário configurar as credenciais Firebase adequadas.
    """
    try:
        ...
        fake_uid = f"fornecedor_{int(datetime.now().timestamp())}"
        
        logger.info(f"📧 Simulando criação de fornecedor: {data['email']}")
        
        return jsonify({
            'sucesso': True,
            'uid': fake_uid,
            'email': data['email'],
            'observacao': 'Fornecedor criado em modo simulação - Firebase Admin SDK não configurado'
        }), 201
...
def require_auth(f):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ud = db.collection('usuarios').document(request.user['uid']).get()
            if ud.exists:
                u = ud.to_dict()
