...
# Importar módulo de administração (COMPATÍVEL - SEM CONFLITOS)
from admin_routes import init_admin_routes, criar_admin_inicial
...
# Configuração do banco PostgreSQL para Render Starter
def get_db_config():
...
def get_db():
    """Conecta ao banco PostgreSQL com cursor de dicionário"""
    try:
        db_config = get_db_config()
        if isinstance(db_config, str):
            # URL de conexão do Render
...
def init_db():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS usuarios (
            ...
            perfil VARCHAR(50) NOT NULL CHECK (perfil IN ('admin', 'requisitante', 'comprador', 'fornecedor')),
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS processos (
            id SERIAL PRIMARY KEY,
            numero VARCHAR(255) UNIQUE NOT NULL,
            tr_id INTEGER REFERENCES termos_referencia(id),
            objeto TEXT NOT NULL,
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS propostas (
            ...
            fornecedor_id INTEGER REFERENCES usuarios(id),
...
# Inicializar módulo de administração (COMPATÍVEL - SEM CONFLITOS)
...
def verify_token():
    ...
    cursor.execute('SELECT id, nome, email, perfil FROM usuarios WHERE id = %s', (request.usuario_id,))
    usuario = cursor.fetchone()
...
def criar_tr():
    ...
    try:
        ...
        cursor.execute('SELECT id FROM usuarios WHERE perfil = %s', ('comprador',))
...
def aprovar_tr(tr_id):
    ...
    try:
        ...
        cursor.execute('''
            ...
            SET status = 'aprovado', parecer = %s
            WHERE id = %s
        ''', (parecer, tr_id))
        
        # Notificar requisitante
        cursor.execute('''
            SELECT usuario_id, numero FROM termos_referencia WHERE id = %s
        ''', (tr_id,))
        tr = cursor.fetchone()
...
def reprovar_tr(tr_id):
    ...
    try:
        ...
        cursor.execute('''
            UPDATE termos_referencia 
            SET status = 'reprovado', motivo_reprovacao = %s
            WHERE id = %s
        ''', (motivo, tr_id))
...
def download_tr_pdf(tr_id):
    ...
    try:
        ...
        cursor.execute('SELECT * FROM termos_referencia WHERE id = %s', (tr_id,))
        tr = cursor.fetchone()
    ...
    try:
        ...
        if request.perfil == 'comprador':
            ...
            cursor.execute('''
                ...
                LEFT JOIN termos_referencia tr ON p.tr_id = tr.id
                JOIN usuarios u ON p.criado_por = u.id
                WHERE cp.fornecedor_id = %s AND p.status = 'aberto'
                ORDER BY p.criado_em DESC
            ''', (request.usuario_id,))
        else:
            conn.close()
...
def convidar_fornecedores(processo_id):
    ...
    try:
        ...
        cursor.execute('SELECT numero FROM processos WHERE id = %s', (processo_id,))
        processo = cursor.fetchone()
...
def criar_proposta():
    ...
    try:
        ...
        cursor.execute('''
            ...
            WHERE processo_id = %s AND fornecedor_id = %s
        ...
        # Notificar comprador
        cursor.execute('''
            SELECT criado_por, numero FROM processos WHERE id = %s
        ''', (data.get('processo_id'),))
        processo = cursor.fetchone()
...
def buscar_proposta_tecnica(cursor, proposta_id):
    ...
    cursor.execute('''
        SELECT * FROM proposta_tecnica WHERE proposta_id = %s
    ''', (proposta_id,))
