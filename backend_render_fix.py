...
# ===== Cadastro de usuários (CNPJ obrigatório p/ fornecedor) =====
import re, bcrypt
from flask import request, jsonify

def _only_alnum(s): return re.sub(r'[^A-Za-z0-9]', '', s or '')

def validar_cnpj_numerico(cnpj: str) -> bool:
    if not re.fullmatch(r'\d{14}', cnpj): return False
    if cnpj == cnpj[0] * 14: return False
    p1=[5,4,3,2,9,8,7,6,5,4,3,2]; p2=[6]+p1
    def dv(c12, pesos):
        s=sum(int(d)*p for d,p in zip(c12,pesos)); r=s%11
        return '0' if r<2 else str(11-r)
    dv1=dv(cnpj[:12],p1); dv2=dv(cnpj[:12]+dv1,p2)
    return cnpj.endswith(dv1+dv2)

@app.route('/api/usuarios', methods=['POST'])
@require_auth
def criar_usuario():
    if request.perfil not in ('comprador','admin'):
        return jsonify({'erro':'Acesso negado'}), 403
    data = request.get_json() or {}
    nome=(data.get('nome') or '').strip()
    email=(data.get('email') or '').strip()
    senha=data.get('senha') or ''
    perfil=(data.get('perfil') or '').lower()
    if not nome or not email or not senha or perfil not in ('comprador','requisitante','fornecedor','admin'):
        return jsonify({'erro':'Campos obrigatórios: nome, email, senha, perfil'}), 400

    cnpj_norm=None
    if perfil=='fornecedor':
        cnpj_raw=data.get('cnpj')
        if not cnpj_raw: return jsonify({'erro':'CNPJ é obrigatório para fornecedor'}), 400
        cnpj_norm=_only_alnum(cnpj_raw)
        if not re.fullmatch(r'\d{14}', cnpj_norm) or not validar_cnpj_numerico(cnpj_norm):
    ...
    if cur.fetchone():
        conn.close(); return jsonify({'erro':'E-mail/usuário já cadastrado'}), 409

    if cnpj_norm:
        try:
            cur.execute('SELECT cnpj FROM usuarios LIMIT 1')
        except Exception:
            try:
                cur.execute('ALTER TABLE usuarios ADD COLUMN cnpj TEXT')
                conn.commit()
            except Exception:
                pass
        cur.execute('SELECT 1 FROM usuarios WHERE cnpj = ?', (cnpj_norm,))
        if cur.fetchone():
            conn.close(); return jsonify({'erro':'CNPJ já cadastrado'}), 409

    senha_hash=bcrypt.hashpw(senha.encode('utf-8'), bcrypt.gensalt())
    cur.execute('INSERT INTO usuarios (nome,email,senha,perfil,ativo,cnpj) VALUES (?,?,?,?,?,?)',
                (nome,email,senha_hash,perfil,1,cnpj_norm))
